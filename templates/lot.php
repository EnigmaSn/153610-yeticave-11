...
if (isset($adv['min_next_bet'])) {
    echo format_sum($adv['min_next_bet']);
            ...
            if (!isset($_SESSION['user']) || $adv['author_id'] === (int) $_SESSION['user']['id']) :
                ?>visually-hidden<?php
            endif; ?> <?= $form_error; ?>" action="/lot.php?id=<?= $adv['id']; ?>" method="post" autocomplete="off">
              <p class="lot-item__form-item form__item <?php if (isset($errors['cost'])) :
